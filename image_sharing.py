...
class ImageSharingBaseHandler(webapp.RequestHandler):
  ...
  def render_to_response(self, filename, template_args):
    ...
    parsed_uri = urlparse( self.request.uri )
    domain = '{uri.scheme}://{uri.netloc}/'.format(uri=parsed_uri)
    template_args.setdefault('domain', domain)
...
class ImageSharingShowJigsawImage(ImageSharingBaseHandler):
  """Handler for viewing a single image.
  ...
  def get(self, pic_key):
    ...
    pic = db.get(pic_key)
    self.render_to_response('show_jigsaw_image.html', {
        'pic': pic,
        'image_key': pic.key(),
    })
...
def main():
  url_map = [('/', ImageSharingAlbumIndex),
             ...
             ('/show_jigsaw_image/([-\w]+)', ImageSharingShowJigsawImage),
